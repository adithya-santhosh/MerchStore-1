...
exports.getCustomerStats = exports.getCustomerById = exports.getAllCustomers = void 0;
...
const getAllCustomers = async (params) => {
    ...
    const spentData = await prisma_1.default.order.groupBy({
        by: ["userId"],
        where: {
            userId: { in: userIds },
    ...
    const spentMap = new Map(spentData.map((s) => [s.userId, Number(s._sum.totalAmount || 0)]));
...
const getCustomerById = async (id) => {
    ...
    if (!user)
        return null;
    // Compute total spent
    const spentResult = await prisma_1.default.order.aggregate({
        where: { userId: id, status: { not: client_1.OrderStatus.CANCELLED } },
        _sum: { totalAmount: true },
    });
    return {
        id: user.id,
...
const getCustomerStats = async () => {
    ...
    const [totalCustomers, newThisMonth, totalAdmins, customersWithOrders] = await Promise.all([
        prisma_1.default.user.count({ where: { role: client_1.UserRole.CUSTOMER } }),
        prisma_1.default.user.count({
            where: { role: client_1.UserRole.CUSTOMER, createdAt: { gte: startOfMonth } },
        }),
        prisma_1.default.user.count({ where: { role: client_1.UserRole.ADMIN } }),
        prisma_1.default.user.count({
            where: { role: client_1.UserRole.CUSTOMER, orders: { some: {} } },
        }),
    ]);
